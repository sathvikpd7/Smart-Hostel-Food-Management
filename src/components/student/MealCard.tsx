...
const MealCard: React.FC<MealCardProps> = ({ 
  ...
  const isTimePassed = () => {
    if (meal.date !== todayStr) {
      return isPastDate; // If not today, just check if it's a past date
    }
    
    // For today's meals, check if current time has passed the cutoff
  ...
  const handleBookMeal = async () => {
    ...
    try {
      ...
      if (pushNotificationService.isEnabled()) {
        pushNotificationService.notifyBookingConfirmed(formatMealTitle(), format(mealDate, 'MMM dd, yyyy'));
        // Schedule reminder notification 30 minutes before meal
        pushNotificationService.scheduleMealNotification(formatMealTitle(), meal.time, 30);
      }
  ...
  const handleCancelBooking = async () => {
    ...
    try {
      ...
      if (pushNotificationService.isEnabled()) {
        pushNotificationService.notifyBookingCancelled(formatMealTitle(), format(mealDate, 'MMM dd, yyyy'));
      }
