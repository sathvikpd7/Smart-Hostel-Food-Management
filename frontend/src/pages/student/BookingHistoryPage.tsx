...
const BookingHistoryPage: React.FC = () => {
  ...
  const safeFormatDate = (value: string, fmt: string) => {
    try {
      if (!value) return '—';
  ...
  const filteredBookings = useMemo(() => {
    if (filterStatus === 'all') return userBookings;
    return userBookings.filter(booking => booking.status === filterStatus);
  }, [filterStatus, userBookings]);
              ...
              <tbody>
                {filteredBookings.length > 0 ? (
                  filteredBookings.map(booking => {
                    const meal = getMealForBooking(booking);
                    return (
                      <tr key={booking.id} className="border-b hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex flex-col">
                            <span className="capitalize font-medium">{booking.type}</span>
                            {meal && meal.menuItems.length > 0 && (
                              <span className="text-xs text-gray-500">{meal.menuItems.slice(0, 2).join(', ')}</span>
                            )}
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          {safeFormatDate(booking.createdAt, 'MMM d, yyyy h:mm a')}
                        </td>
                        <td className="px-4 py-3">
